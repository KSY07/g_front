...
export const ConstructionExampleCreateModal = ({onClose}:CreateBoardTipModalProps) => {
  ...
  const onClickSubmit = () => {
    ...
    axios.post('/board/constructionExample', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      }
    }).then(response => {
      if(response.status === 201) {
        toast("시공 사례 작성이 완료되었습니다. 새로고침 해주세요.");
        router.refresh();
        onClose();
      }
    }).catch(error => {
          ...
          <>
            <ModalBody>
              ...
              <div className="overflow-y-auto flex justify-center">
                <ReactQuill placeholder="시공 사례를 작성해주세요."
